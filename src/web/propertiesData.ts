...
const propertiesData = {
    ...
    'network-compression-threshold': _p(false, '256', 'デフォルトでは、n-1バイトの大きさのパケットは正常に送信できるが、nバイト以上のパケットは圧縮されるため、数値が小さいほど圧縮率は高くなるが、少量のバイトを圧縮すると、実際には入ったものよりも大きな結果になる可能性がある'),
    ...
    'view-distance': _p(false, '10', '	サーバーがクライアントに送信するワールドデータの量を、プレーヤーの各方向のチャンク（直径ではなく半径）で測定して設定する'),
